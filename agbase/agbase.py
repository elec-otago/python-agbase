...
class AgBase:
    ...
    def __api_call(self, http_verb, route, data=None, query_params=None):
        ...
        if data is None and query_params is None:
            response = http_call(self.agbase_ip_address + route)
        elif data is None and query_params is not None:
            response = http_call(self.agbase_ip_address + route, params=query_params)
        elif query_params is None and data is not None:
            response = http_call(self.agbase_ip_address + route, data=json.dumps(data))
        else:
            response = http_call(self.agbase_ip_address + route, params=query_params, data=json.dumps(data))
    ...
    def __auth_user(self, email, pwd):
        ...
        post_response = self.session.post(self.agbase_ip_address + "auth/", data=json.dumps(user_details))
    ...
    def connect(self, email, pwd, agbase_ip):
        self.currentUser = email
        self.currentPwd = pwd
        self.agbase_ip_address = agbase_ip
